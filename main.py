...
for k in range(2, iterations):
    last_volume = np.array([float(df['volume'][k-2]) for df in list(klines.values())])[:, np.newaxis] # volumes at k-1 and k when Y = df['close']
    current_volume = np.array([float(df['volume'][k-1]) for df in list(klines.values())])[:, np.newaxis]
    ...
    if k == iterations - 1:
        ...
        diff = (Y-X)/np.linalg.norm(Y-X)
        print('MSE of normalized data = ', ((diff).T @ (diff))/(len(Y)-1))
